/*
  BUG-110834: Button style fix for french (D562965)
  CONG-112: Add global variable funnel_url for context logging (D561755)
  Add logic for browser_extension_image_multiselect_1 (D559013)
  Log CONG-102: (5) Select board w sections (D558911)
  Log CONG-103: (6) Select Board w/o sections (D558911)
  Log CONG-107: (6) Select existing section (D558911)
  Log Start and End Image Picker (D557938)
  Add funnel_uuid and url to all core frontend_events (D556658)
  Experiment set up for UI refresh for browser_extension_image_multiselect_1 (D558241)
  Convert image data to array for multiselect (D555166)
  Remove X from follow nag (BRP-5399)
*/
    ...
    f: {
      ...
      mouseover: (e) => {
        ...
        if (el.className === $.a.k + "_mask") {
          ...
          $.v.experimentGroup["browser_extension_image_multiselect_1"] &&
          $.v.experimentGroup["browser_extension_image_multiselect_1"] !==
            "control"
        ) {
          // experiment: browser_extension_image_multiselect_1
          if (el.classList.contains($.a.k + "_outerAddFormOpener")) {
            $.f.changeClass({ el: $.v.selectedItem, remove: "selected" });
          }
        }
      },
        ...
        switch (cmd) {
          ...
          case "save":
            ...
            if (viewType) {
              ...
              dataAttributes.element = isPinningToSection
                ? "SECTION_COVER"
                : "BOARD_COVER";
            }
            break;
      ...
      act: {
        ...
        newPinWin: (result) => {
          ...
          if (
            ...
            if (result.pin.media) {
              ...
              $.d
                .getElementById("col_" + ($.v.savedPins % numColumns))
                .appendChild(img);
            }
          ...
          if (
            myPromoButton.show_promote_button &&
            myPromoButton.promote_button_destination &&
            myPromoButton.promote_button_text
          ) {
          ...
          // experiment will activate in the background and send a True here if we're eligible
          let myGroup = (
            ((result || {}).experiment || {})
              .browser_extension_send_after_save_1 || {}
          ).group;
          if (myGroup) {
            $.f.debug(
              "Group received for browser_extension_send_after_save_1."
            );
            if (myGroup !== "control") {
              $.f.debug("Group found with view eligibility: " + myGroup);
              // send the view to the background
              $.f.send({
                to: "background",
                act: "contextLog",
                data: {
                  eventType: "VIEW",
                  viewType: "SEND_AFTER_SAVE"
                }
              });
              // update button
              $.s.feedbackButtonSend.innerText = $.v.msg.msgSendAfterSave;
              $.s.feedbackButtonSend.dataset.url = `${seeItNowUrl}?share=true`;
              // reveal button
              $.f.changeClass({ el: $.s.feedbackButtonSend, remove: "hidden" });
            } else {
              $.f.debug("Control group found, no view.");
            }
          } else {
            $.f.debug(
              "No group received for browser_extension_send_after_save_1"
            );
          }
        ...
        renderFollowNag: (input) => {
          // get our group, which might be undefined or contain a string like "control" or "treatment"
          let myGroup = (input || {}).group;
          // log the view of the board picker and the follow nag, even though we may not be showing the nag for the control group
          if (myGroup) {
            ...
            if ((((input.data || {}).output || {}).response || {}).data) {
              ...
              if (!pinner.followed_by_me && !pinner.blocked_by_me) {
                ...
                // only show if we are not in the control group
                // ATTENTION FUTURE ME: this is where you will add more groups
                if (myGroup !== "control") {
                  // add pinner avatar
                  $.s.followAvatar.style.backgroundImage =
                    'url("' + pinner.image_medium_url + '")';
                  // add pinner name
                ...
                $.f.debug(
                  "Following  or blocking " + pinner.full_name + "; ignoring."
                );
              }
            } else {
        ...
        populateCreateForm: (o) => {
          const renderCreateForm = () => {
            ...
            // experiment: browser_extension_image_multiselect_1
            let expName = "browser_extension_image_multiselect_1";
            // get our group, which might be undefined or contain a string like "control", "enabled_ui_refresh" or "enabled_multiselect"
            let myGroup = o.experimentGroup[expName];
            if (myGroup && myGroup !== "control") {
              // alter layout to feedback
              $.f.changeClass({ el: $.s.main, add: "mainMultiselect" });
            }
            $.v.experimentGroup[expName] = myGroup;
          };
      ...
      filter: () => {
        ...
        if (!$.v.hazSelectBlock) {
          ...
          dupeCheck = () => {
            ...
            if (
              mustNotMatch.filter((item) => item === fv.toLowerCase()).length
            ) {
              hazDupe = true;
            }
            return hazDupe;
  ...
  structure: {
    main: {
      ...
      feedback: {
        ...
        feedbackFooter: {
          // buttons (see it now, close, send)
          feedbackButtonClose: {
            addClass: "button cancel",
            cmd: "close"
          },
          // experiment: browser_extension_send_after_save_1
          feedbackButtonSend: {
            addClass: "button send hidden",
